# -*- coding: utf-8 -*-
"""全内存搜目标字符串（VirtualQueryEx 枚举提交可读页）"""
import sys, ctypes
from ctypes import wintypes
sys.path.insert(0, r"C:\Users\abps\Desktop\DiabloIIToolbox\Py")
import mem_read as mem

kernel32 = ctypes.WinDLL("kernel32", use_last_error=True)
MEM_COMMIT = 0x1000
PAGE_READABLE = 0x02 | 0x04 | 0x08 | 0x10 | 0x20 | 0x40 | 0x80


class MEMORY_BASIC_INFORMATION(ctypes.Structure):
    _fields_ = [
        ("BaseAddress", ctypes.c_void_p),
        ("AllocationBase", ctypes.c_void_p),
        ("AllocationProtect", wintypes.DWORD),
        ("RegionSize", ctypes.c_size_t),
        ("State", wintypes.DWORD),
        ("Protect", wintypes.DWORD),
        ("Type", wintypes.DWORD),
    ]


pid = mem.find_process("D2Loader.exe")
print("PID", pid)
if not pid:
    sys.exit()
h = mem.open_process_readonly(pid)

needles = {
    "大血瓶u8": "大血瓶".encode("utf-8"),
    "大血瓶u16": "大血瓶".encode("utf-16-le"),
    "完美红宝石u8": "完美红宝石".encode("utf-8"),
    "完美红宝石u16": "完美红宝石".encode("utf-16-le"),
    "治疗药剂u8": "治疗药剂".encode("utf-8"),
    "小护身符u8": "小护身符".encode("utf-8"),
}

addr = 0x10000
regions = []
while addr < 0x7FFFFFFF:
    mbi = MEMORY_BASIC_INFORMATION()
    r = kernel32.VirtualQueryEx(h, ctypes.c_void_p(addr), ctypes.byref(mbi),
                                ctypes.sizeof(mbi))
    if not r:
        break
    size = mbi.RegionSize or 0x1000
    if mbi.State == MEM_COMMIT and (mbi.Protect & 0xFF) & PAGE_READABLE:
        regions.append((int(mbi.BaseAddress), int(size)))
    addr = (int(mbi.BaseAddress) + size + 0xFFFF) & ~0xFFFF

print("可读区域数", len(regions))
found = {k: [] for k in needles}
for lo, size in regions:
    # 分块读 1MB
    chunk = 0x100000
    for off in range(0, size, chunk):
        n = min(chunk, size - off)
        buf = mem.read(pid, h, lo + off, n)
        if not buf:
            continue
        for k, nd in needles.items():
            if len(found[k]) >= 6:
                continue
            p = 0
            while True:
                i = buf.find(nd, p)
                if i < 0:
                    break
                found[k].append(lo + off + i)
                p = i + 1

for k, v in found.items():
    print(k, [hex(x) for x in v] if v else "未找到")
ctypes.windll.kernel32.CloseHandle(h)
