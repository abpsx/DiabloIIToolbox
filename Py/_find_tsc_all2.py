# -*- coding: utf-8 -*-
"""全内存搜 'tsc ' 全部命中，找 0x1A8 行宽对齐的 GetItemText 大表"""
import sys, ctypes
...
for hit in hits:
    ...
    for k in (-2, -1, 1, 2):
        ...
        if code and all(32 <= b < 127 for b in code) and code[:1].isalpha():
            ok += 1
    checked.append((rs, ok, hit))

for rs, ok, hit in sorted(checked, key=lambda x: -x[1]):
    if ok >= 3:
        print(f"  {hit:#x}: 行首{rs:#x} 连续行={ok}  <- 疑似大表")
    else:
        print(f"  {hit:#x}: 行首{rs:#x} 连续行={ok}")
