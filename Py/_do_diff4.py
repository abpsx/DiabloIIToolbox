# -*- coding: utf-8 -*-
"""综合 diff：找 ±1 页数候选（byte/word/dword）"""
...
old = json.load(open(r"C:\Users\abps\Desktop\DiabloIIToolbox\Py\_diff_snap2.json", encoding="utf-8"))

byte_c, word_c, dword_c = [], [], []
for k, hexs in old.items():
    ...
    n = len(ob)
    for i in range(n):
        if ob[i] != nb[i] and 1 <= ob[i] <= 200 and 1 <= nb[i] <= 200 and abs(nb[i] - ob[i]) == 1:
...
for a, o, v in byte_c[:30]:
    print(f"  0x{a:X}: {o} -> {v}")
print(f"word ±1: {len(word_c)}")
for a, o, v in word_c[:30]:
    print(f"  0x{a:X}: {o} -> {v}")
print(f"dword ±1: {len(dword_c)}")
for a, o, v in dword_c[:30]:
    print(f"  0x{a:X}: {o} -> {v}")
